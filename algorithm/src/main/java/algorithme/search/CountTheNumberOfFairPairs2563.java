    ...
    class Solution {
        public long countFairPairs(int[] nums, int lower, int upper) {
            ...
            for (int i = 0; i < nums.length; i++) {
                int leftBound = searchIndex(nums, i, lower - nums[i], true);
                int rightBound = searchIndex(nums, i, upper - nums[i], false);
                ans += (rightBound - leftBound);
            }
            return ans;
        }

        private int searchIndex(int[] nums, int index, int target, boolean left) {
            int from = 0;
            int to = index;
            while (from < to) {
                int mid = (to + from) / 2;
